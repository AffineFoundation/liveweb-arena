...
def test_initialize_cache_fetches_and_writes_cache(tmp_path, monkeypatch):
    ...
    async def _fake_fetch_all_subnets() -> Dict[str, Any]:
        return {"subnets": {"10": {"emission": 3.0}, "20": {"emission": 7.0}}}
...
async def test_fetch_all_subnets_retries_on_server_error_then_succeeds(monkeypatch):
    ...
    async def _fake_client_session(*_args, **_kwargs):
...
def test_safe_float_basic_cases():
    # None and empty values -> None
    assert taostats_api._safe_float(None) is None
    assert taostats_api._safe_float("") is None
...
def test_parse_subnet_data_conversions_and_defaults():
    subnet = {
        "netuid": 27,
        "latest_snapshot": {
            "subnet_identities_v3": {"subnetName": "Gittensor"},
            "token_symbol": "GT",
            "subnet_tao": str(5 * taostats_api.RAO_TO_TAO),
            "subnet_alpha_in": str(10 * taostats_api.RAO_TO_TAO),
            "subnet_volume": str(2 * taostats_api.RAO_TO_TAO),
...
def test_parse_subnet_data_missing_fields_and_name_fallback():
    # No identities, minimal snapshot; name should fall back to token_symbol then SN{netuid}
    subnet = {
        "netuid": 10,
        "latest_snapshot": {
            "token_symbol": "SN10SYM",
            "subnet_tao": None,
            "subnet_alpha_in": "",
            "subnet_tao_in_emission": None,
        },
    ...
    assert parsed["netuid"] == 10
    # Falls back to token_symbol
    assert parsed["name"] == "SN10SYM"
    # Missing numeric fields stay None
    assert parsed["tao_in"] is None
    assert parsed["alpha_in"] is None
    assert parsed["emission"] is None
    # Market cap cannot be computed (no price / alpha_out)
    assert parsed["market_cap"] is None
...
def test_normalize_emission_converts_small_totals_to_percentages():
    ...
    # Entry with None emission should remain None
    assert result["3"]["emission"] is None


def test_normalize_emission_skips_when_total_zero_or_large():
    # Empty input
    assert taostats_api._normalize_emission({}) == {}

    # Total == 0 (all None) -> no change
    subnets_zero = {"1": {"emission": None}}
    assert taostats_api._normalize_emission(subnets_zero) is subnets_zero

    # Total >= 50 -> treated as already-percentages, no change
    subnets_pct = {"1": {"emission": 60.0}, "2": {"emission": 40.0}}
    result = taostats_api._normalize_emission(subnets_pct)
    assert result is subnets_pct


def test_filter_by_emission_selects_top_half_and_handles_missing():
    subnets = {
        "1": {"emission": 10.0},
        "2": {"emission": None},  # treated as -1
        "3": {"emission": 5.0},
...
def test_file_cache_helpers_round_trip(tmp_path, monkeypatch):
    cache_dir = tmp_path / "cache-root"
    monkeypatch.setenv("LIVEWEB_CACHE_DIR", str(cache_dir))
    monkeypatch.setenv("LIVEWEB_CACHE_TTL", "3600")
...
def test_file_cache_invalid_when_expired(tmp_path, monkeypatch):
    cache_dir = tmp_path / "cache-root-expired"
    monkeypatch.setenv("LIVEWEB_CACHE_DIR", str(cache_dir))
    # Very small TTL
    monkeypatch.setenv("LIVEWEB_CACHE_TTL", "1")
...
def test_file_cache_handles_corrupt_json(tmp_path, monkeypatch):
    cache_dir = tmp_path / "cache-root-corrupt"
    monkeypatch.setenv("LIVEWEB_CACHE_DIR", str(cache_dir))
...
def test_initialize_cache_uses_existing_file_cache(tmp_path, monkeypatch):
    # Ensure fresh context cache
    taostats_api._subnet_cache.set(None)
    ...
    # If initialize_cache tries to hit the network, fail the test
    async def _fail_fetch_all_subnets() -> Dict[str, Any]:
        raise AssertionError("fetch_all_subnets should not be called when cache exists")
    ...
    cached = taostats_api.get_cached_subnets()
    # Filter keeps top half by emission -> 1 and 2
    assert set(cached.keys()) == {"1", "2"}


def test_initialize_cache_fetches_and_writes_file(tmp_path, monkeypatch):
    # Fresh context cache
    taostats_api._subnet_cache.set(None)
    ...
    # Ensure no file exists so initialize_cache must fetch
    cache_file = taostats_api._get_file_cache_path()
    if cache_file.exists():
        cache_file.unlink()
    ...
    cached = taostats_api.get_cached_subnets()
    # After filtering, both entries remain (top half of 2)
    assert set(cached.keys()) == {"10", "20"}

    # File cache should have been written
    assert cache_file.exists()
    raw = json.loads(cache_file.read_text())
    assert "subnets" in raw and raw["subnets"]
